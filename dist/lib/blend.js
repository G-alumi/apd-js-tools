...
export function hslToRgb(h, s, l) {
    ...
    let x = 1;
    if (max + l > 1)
        x = Math.min(x, (1 - l) / max);
    if (min + l < 0)
        x = Math.min(x, l / (-min));
    const clamp = (v) => Math.min(255, Math.max(0, Math.round((v * x + l) * 255)));
    return [clamp(r), clamp(g), clamp(b)];
}
function ch(v) { return Math.min(255, Math.max(0, v)); }
const blendFns = {
    // 通常
    ...
    0x05: (sr, sg, sb, dr, dg, db) => {
        const f = (s, d) => d < 128 ? s * d / 128 : 255 - (255 - d) * (255 - s) / 128;
    ...
    0x07: (sr, sg, sb, dr, dg, db) => {
        const f = (s, d) => s < 128 ? s * d / 128 : 255 - (255 - d) * (255 - s) / 128;
        return [f(sr, dr), f(sg, dg), f(sb, db), false];
    },
    // 覆い焼き
    0x08: (sr, sg, sb, dr, dg, db) => {
        const f = (s, d) => s === 255 ? 255 : ch((d * 255 / (255 - s)) | 0);
        return [f(sr, dr), f(sg, dg), f(sb, db), false];
    },
    // 焼き込み
    0x09: (sr, sg, sb, dr, dg, db) => {
        const f = (s, d) => s === 0 ? 0 : ch(255 - (((255 - d) * 255 / s) | 0));
        return [f(sr, dr), f(sg, dg), f(sb, db), false];
    },
    // 焼き込みリニア
    ...
    0x0D: (sr, sg, sb, dr, dg, db) => {
        const f = (s, d) => s >= 128 ? Math.max(s * 2 - 255, d) : Math.min(s * 2, d);
        return [f(sr, dr), f(sg, dg), f(sb, db), false];
    },
    // 差の絶対値
